...
const menuLeftItems = ["menu_settings", "menu_profile"];
const menuRightItems = ["menu_discord", "menu_website"];
...
const bgColors = [
  "https://raw.githubusercontent.com/slackerish/HYPE-ENGINE-v0.1/main/hype/images/menuBGmagenta.png",
  "https://raw.githubusercontent.com/slackerish/HYPE-ENGINE-v0.1/main/hype/images/menuBGblue.png"
];
...
document.addEventListener("click", (e) => {
  if (e.target === hitbox || hitbox.contains(e.target)) startGame();
});
...
function changeSelection(change) {
  ...
  if (currentMenuIndex < 0) currentMenuIndex = menuSprites.length - 1;
  if (currentMenuIndex >= menuSprites.length) currentMenuIndex = 0;
...
function flashBackground() {
  // Alternate between magenta and blue
  currentBgIndex = (currentBgIndex + 1) % bgColors.length;
  mainBg.style.backgroundImage = `url("${bgColors[currentBgIndex]}")`;
}
...
function updateMenuLayout() {
  ...
  menuSprites.forEach((sprite, index) => {
    ...
    // Revolver logic: Wrap around visually
    if (diff > menuSprites.length / 2) diff -= menuSprites.length;
    if (diff < -menuSprites.length / 2) diff += menuSprites.length;
    
    const targetY = centerY + (diff * (itemHeight + gap));
    sprite.container.style.top = `${targetY}px`;
    
    // Only show 5 items (Current + 2 above + 2 below)
    if (Math.abs(diff) <= 2) {
...
hitbox.addEventListener("mouseenter", () => {
  if (!hasStarted) bgContainer.classList.add("zoomed");
});

hitbox.addEventListener("mouseleave", () => {
  if (!hasStarted) bgContainer.classList.remove("zoomed");
});
...
class MenuSprite {
  ...
  async loadResources() {
    const basePath = "https://raw.githubusercontent.com/slackerish/HYPE-ENGINE-v0.1/main/assets/images/mainmenu/";
    const menuName = this.name;
    
    this.image = new Image();
    this.image.src = `${basePath}menu_${menuName}.png`;
    await this.loadXML(`${basePath}menu_${menuName}.xml`, menuName);
    this.animate();
  }
  ...
  async loadXML(url, menuName) {
    try {
      ...
      for (let i = 0; i < subTextures.length; i++) {
        const name = subTextures[i].getAttribute("name");
        const frameData = {
        ...
        if (name.includes(`${menuName} basic`)) this.frames.basic.push(frameData);
        else if (name.includes(`${menuName} white`)) this.frames.white.push(frameData);
      }
    } catch (e) {
  ...
  animate(timestamp = 0) {
    ...
    if (delta > 1000 / this.fps) {
      ...
      const cur = this.selected ? this.frames.white : this.frames.basic;
      if (cur.length > 0) this.currentFrame = (this.currentFrame + 1) % cur.length;
    }
  ...
  draw() {
    const cur = this.selected ? this.frames.white : this.frames.basic;
    if (cur.length === 0 || !this.image.complete) return;
    
    const frame = cur[this.currentFrame];
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    
    const scale = Math.min(this.canvas.width / frame.width, this.canvas.height / frame.height);
    ...
    this.ctx.drawImage(
      ...
      (this.canvas.width - dw) / 2, (this.canvas.height - dh) / 2,
      dw, dh
    );
  }
  ...
  setSelected(selected) {
    ...
    if (selected) this.container.classList.add("selected");
    else this.container.classList.remove("selected");
  }
}
...
class CornerSprite {
  constructor(name, parentContainer) {
    ...
    this.container = document.createElement("div");
    this.container.className = "corner-item";
    this.canvas = document.createElement("canvas");
    ...
    try {
      ...
      for (let i = 0; i < subTextures.length; i++) {
        ...
        if (name.startsWith("idle")) {
          ...
          this.frames.selected.push(frameData);
        }
      }
    } catch (e) {
      console.error("Corner XML load error:", e);
    }
  }
  ...
  animate(timestamp = 0) {
    ...
    if (delta > 1000 / this.fps) {
      ...
      const cur = this.selected ? this.frames.selected : this.frames.idle;
      if (cur.length > 0) {
        this.currentFrame = (this.currentFrame + 1) % cur.length;
      }
    }
...
function initializeMenu() {
  menuItems.forEach((item, index) => {
    const sprite = new MenuSprite(item, index);
    menuSprites.push(sprite);
  });
  updateMenuSelection();
}

function initializeCornerMenus() {
  const leftContainer = document.getElementById("leftCorner");
  const rightContainer = document.getElementById("rightCorner");
  
  menuLeftItems.forEach(item => {
    const sprite = new CornerSprite(item, leftContainer);
    leftSprites.push(sprite);
  });
  
  menuRightItems.forEach(item => {
    const sprite = new CornerSprite(item, rightContainer);
...
function updateMenuSelection() {
  menuSprites.forEach((sprite, index) => {
    sprite.setSelected(index === currentMenuIndex);
  });
}
...
function selectCurrentMenuItem() {
  ...
  const selectedItem = menuSprites[currentMenuIndex];
  selectedItem.container.classList.add("confirming");
  ...
  setTimeout(() => {
    // isSelecting = false;
    // selectedItem.container.classList.remove("confirming");
  }, 2000);
}
...
introImg.src = "https://raw.githubusercontent.com/slackerish/HYPE-ENGINE-v0.1/main/hype/images/HYPEINITUALIZE.png";

let introFrames = [];
let introIdx = 0;

fetch("https://raw.githubusercontent.com/slackerish/HYPE-ENGINE-v0.1/main/hype/images/HYPEINITUALIZE.xml")
  .then(r => r.text())
  .then(txt => {
    const xml = new DOMParser().parseFromString(txt, "application/xml");
    const texs = xml.getElementsByTagName("SubTexture");
    ...
    const loopIntro = (ts) => {
      if (introFrames.length > 0) {
        ...
        const s = Math.max(canvas.width / f.width, canvas.height / f.height);
        const dw = f.width * s;
        const dh = f.height * s;
        ...
        introIdx = (introIdx + 1) % introFrames.length;
      }
      requestAnimationFrame(loopIntro);
    };
    
    introImg.onload = () => requestAnimationFrame(loopIntro);
  });
